...
Executable creates a server that will connect with up to 5 concurrent clients and
	...
	3. encode cyphertext using key and return to client
...
// file to encrypt data on a given socket and return it
void encrypt(int socket) {
	...
	charsRead = send(socket, "E", 2, 0);
	if (charsRead < 0) error("SERVER: send");
	if (strcmp(buffer, "E") != 0) {fprintf(stderr, "Server:Handshake Error\n"); exit(1);}
