...
list1 = re.findall(r'"(.*?)"', open('p042_words.txt').read())

triangle = list((n * (n + 1)) / 2 for n in range(1, 100))

def triangle_word(word):
	value = 0
	for char in word: value += (ord(char) - 64)
	if value in triangle: return True
	else: return False

total = 0

for word in list1: 
	if triangle_word(word) == True: 
		total += 1
